...
Trước khi vẽ architecture hoặc implement, Claude phải lấy đủ context từ project/rules/user request và xác nhận lại cách hiểu với user.
...
Claude phải đọc đủ context liên quan trong target project trước khi đề xuất architecture:
...
Trước Step 01, Claude phải giải thích lại:
...
Nếu prompt của user thiếu case quan trọng, Claude phải bổ sung vào phân tích trước khi architecture.
...
Claude phải nêu hướng giải quyết dự kiến và kiểm tra xem có khớp với ý user không:
...
- [ ] Đã tóm tắt lại yêu cầu theo cách hiểu của Claude
