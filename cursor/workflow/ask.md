...
Do not use Ask when the user explicitly asks to edit, implement, refactor, fix, commit, or publish changes. Route those requests to the matching command workflow.
...
- A specific Cursor skill or workflow
...
- For ai-skills questions, read the matching `cursor/` rule, workflow, template, or skill.
...
- Feature or code generation -> `cursor-implement`
- Bug fix -> bug-fix workflow when available, otherwise `cursor-implement` with bug-fix scope
- Refactor -> refactor workflow when available, otherwise `cursor-implement` with refactor scope
- Code review -> `cursor-review`
- Skill/rule update -> `cursor-update-skill` or the relevant rule-update command
