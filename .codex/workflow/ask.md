...
- A specific Codex skill or workflow
...
- For ai-skills questions, read the matching `.codex/` rule, workflow, template, or skill.
...
- Feature or code generation -> `codex-implement`
- Bug fix -> bug-fix workflow when available, otherwise `codex-implement` with bug-fix scope
- Refactor -> refactor workflow when available, otherwise `codex-implement` with refactor scope
- Code review -> `codex-review`
- Skill/rule update -> `codex-update-skill` or the relevant rule-update command
